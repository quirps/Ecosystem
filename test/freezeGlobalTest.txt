...
const {
  ...
  const { deployFrozenGlobal } = require('../scripts/deployFrozenGlobal.js')
  const { ethers } = require("hardhat");
  const { assert, expect } = require('chai');
  const { hexStripZeros } = require('ethers/lib/utils.js');

  
  describe('FrozenGlobalDiamondTest', async function () {
    let diamondAddress
    let frozenOwnerCut
    let tx
    let receipt
    ...
    before(async function () {
      ...
      await deployFrozenGlobal(diamondAddress)
      frozenGlobalCut = await ethers.getContractAt('IFreezeGlobal', diamondAddress)
      diamondLoupeFacet = await ethers.getContractAt('IDiamondLoupe',diamondAddress)
    })
  
    it('Freeze Global, check diamond selectors were cached and only remaining are global selectors', async () => {
      let _globalSelectors;
      let _frozenSelectors;
      let _freezeDuration = 2;
      let _facetAddresses;
      let _unfreezeTx;
      let _unfreezeTxLogs;

      await frozenGlobalCut.freezeGlobal(_freezeDuration);
      
      _globalSelectors = await frozenGlobalCut.callStatic.freezeGlobalSelectors()
      _frozenSelectors = await frozenGlobalCut.callStatic.freezeCachedSelectors()
      await expect( frozenGlobalCut.unFreezeGlobal() )
         .to.be.revertedWith("Freeze duration hasn't expired yet.") ;
      //await expireTime, check selectors are all back.   
      await new Promise(resolve => setTimeout(resolve, _freezeDuration));
      
      _unfreezeTx = await frozenGlobalCut.unFreezeGlobal();
      _unfreezeTxLogs = await _unfreezeTx.wait()
      _facetAddresses = await diamondLoupeFacet.callStatic.facetAddresses();
      console.log(3)
