...
contract TicketExchange is ITicketExchange, Ownable, ReentrancyGuardContract, ERC1155Holder {
   ...
   // --- NEW: Time-Delayed Currency Proposals ---
     ...
     * @param _registryAddress The address of the deployed EcosystemRegistry contract.

     */

    function setEcosystemRegistry(address _registryAddress) external onlyOwner {

        require(address(ecosystemRegistry) == address(0), "Registry already set");

        require(_registryAddress != address(0), "Zero address");

        ecosystemRegistry = IEcosystemRegistry(_registryAddress);
    ...
    function createSale(
        ...
        DistributionShare[] calldata distribution // <-- NEW PARAMETER

    ) external override {

        // --- Validation ---

        require(trustedTokens[paymentTokenAddress] > uint32(block.timestamp), "Payment token not trusted");

        require(trustedTokens[ecosystemAddress] > uint32(block.timestamp), "Ticket not trusted");
        ...
        uint256 totalDistributionBp = 0;

        for (uint i = 0; i < distribution.length; i++) {

            if (distribution[i].recipient == address(0)) revert ZeroAddress();

            totalDistributionBp += distribution[i].basisPoints;

        }

        if (totalDistributionBp > BASIS_POINTS_DIVISOR) {

            revert("Total distribution exceeds 100%");
        ...
        sales[saleId] = SaleInfo({
            ...
            distribution: distribution // <-- POPULATE NEW FIELD
            ...
            if (sale.distribution.length > 0) {
                ...
                for (uint i = 0; i < sale.distribution.length; i++) {
                    ...
                    if (amountToSend > 0) {
                        ...
                        paymentToken.safeTransfer(share.recipient, amountToSend);
                ...
                if (remainder > 0) {
                    ...
                    paymentToken.safeTransfer(creator, remainder);
