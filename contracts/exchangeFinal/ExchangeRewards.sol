...
import "./interfaces/ITokenSaleStaking.sol"; // Assuming path is correct
...
contract ExchangeRewards is IExchangeRewards, Ownable, ReentrancyGuardContract, IERC1155Receiver {
    ...
    ITokenSaleStaking public tokenSaleStakingContract; // Correct type for the contract
    ...
    uint256 public txOriginFeeShareBasisPoints = 30;
    ...
    error InitatorFeeBoundExceeded();
    // --- Constructor ---
   ...
   /**
    ...
    function recordFee(
        ...
        console.logUint(987); // This prints
        // Ensure msg.sender is the TicketExchange contract
        if (msg.sender != ticketExchange) revert NotTicketExchange();
        console.logUint(876); // This prints
        if (platformFeeAmount == 0) return; // No fee to record

        uint256 remainingFee = platformFeeAmount; // Start with the total platform fee
        IERC20 token = IERC20(paymentToken); // Get ERC20 token instance

        // --- DEBUGGING LOGS START IN SOLIDITY (ExchangeRewards.recordFee) ---
        console.log("ER.recordFee: Entry values");
        console.logAddress(address(this)); // Address of ExchangeRewards
        console.logAddress(paymentToken); // Address of the payment token
        console.logUint(platformFeeAmount); // Total fee amount RECEIVED by recordFee
        console.logUint(token.balanceOf(address(this))); // Balance of paymentToken held by ExchangeRewards *before* any transfers
        console.logAddress(txInitiator); // txInitiator received
        // --- DEBUGGING LOGS END ---

        // --- 1. Allocate share to the Transaction Initiator ---
        // Only if a share is configured and the initiator is not the zero address
        if (txOriginFeeShareBasisPoints > 0 && txInitiator != address(0)) { 
            uint256 initiatorShare = (platformFeeAmount * txOriginFeeShareBasisPoints) / BASIS_POINTS_DIVISOR;

            // --- DEBUGGING LOGS (Initiator Share Calculation) ---
            console.log("ER.recordFee: Initiator Share Calculation");
            console.logUint(txOriginFeeShareBasisPoints);
            console.logUint(BASIS_POINTS_DIVISOR);
            console.logUint(initiatorShare);
            // --- END DEBUGGING LOGS ---

            // Ensure initiatorShare is positive and doesn't exceed the remaining fee
            if (initiatorShare > 0 && initiatorShare <= remainingFee) {
                remainingFee -= initiatorShare; // Deduct share from remaining fee
                
                console.logUint(76543);
                console.logAddress(txInitiator); // Destination for initiator share
                console.logUint(initiatorShare); // Amount to send to initiator
                
                // Standard ERC20 transfer with a require check
                require(token.transfer(txInitiator, initiatorShare), "ERC20: initiator share transfer failed");
                console.logUint(765); // This will print if transfer succeeds
            }
        }

        // --- 2. Allocate share to the TokenSaleStaking contract ---
        // Only if the contract is set and a share is configured
        if (address(tokenSaleStakingContract) != address(0) && tokenSalePoolFeeShareBasisPoints > 0) {
            uint256 salePoolShare = (platformFeeAmount * tokenSalePoolFeeShareBasisPoints) / BASIS_POINTS_DIVISOR;

            // --- DEBUGGING LOGS (Sale Pool Share Calculation) ---
            console.log("ER.recordFee: Sale Pool Share Calculation");
            console.logUint(tokenSalePoolFeeShareBasisPoints);
            console.logUint(salePoolShare);
            // --- END DEBUGGING LOGS ---

            // Ensure salePoolShare is positive and doesn't exceed the remaining fee
            if (salePoolShare > 0 && salePoolShare <= remainingFee) {
                remainingFee -= salePoolShare; // Deduct share from remaining fee
                console.logUint(12345);
                
                // Standard ERC20 transfer with a require check
                require(token.transfer(address(tokenSaleStakingContract), salePoolShare), "ERC20: sale pool transfer failed");
                console.logUint(6789);
                
                // Corrected call: Use ITokenSaleStaking interface and correct 2-argument signature
                // The recordFee function on TokenSaleStaking does not need the txInitiator
                tokenSaleStakingContract.recordFee(paymentToken, salePoolShare); 
            }
        }

        // --- 3. Remaining fee to main staking pool (Fee Share Rewards) ---
        // This remaining portion is added to the accumulator for the main staking pool.
        // This is an internal accounting update, not an external transfer.
        if (remainingFee > 0) {
            // Assuming rewardInfo is a mapping where paymentToken (address) maps to RewardInfo struct
            // and RewardInfo contains rewardPerTokenStored and totalStaked.
            // The key for rewardInfo is `uint256` (tokenId), so we need to convert paymentToken address.
            // This is based on your previous `rewardInfo[uint256(uint160(paymentToken))]` pattern.
            RewardInfo storage stakingInfo = rewardInfo[uint256(uint160(paymentToken))];
            if (stakingInfo.totalStaked > 0) {
                stakingInfo.rewardPerTokenStored += (remainingFee * PRECISION_FACTOR) / stakingInfo.totalStaked;
            } else {
                // If no tokens are staked in the main pool, the fees might accumulate here
                // or be sent to a treasury. For now, they just stay in the contract.
                // You might want to add a mechanism to sweep these if totalStaked is 0.
            }
        }
        
        // Emit the FeeRecorded event with the original total platform fee received
        emit FeeRecorded(paymentToken, platformFeeAmount);
    }

    // ... (rest of your ExchangeRewards functions, ensuring all safeTransfer/safeTransferFrom are replaced
    //      with direct transfer/transferFrom + require checks, as provided in the previous full response) ...
    ...
    function unstake(uint256 stakeId) external override ReentrancyGuard {
        ...
        if (pendingFeeShare > 0) {
            address paymentToken = address(uint160(tokenId));
            IERC20(paymentToken).transfer(user, pendingFeeShare);
    ...
    function claimFeeShareRewards(uint256[] calldata stakeIds) external override ReentrancyGuard {
        ...
        for (uint i = 0; i < uniqueTokenCount; ++i) {
            ...
            if (amountToTransfer > 0) {
                ...
                IERC20(paymentToken).transfer(user, amountToTransfer);
...
/** @inheritdoc IExchangeRewards*/
    ...
    function claimHoldingReward(uint256 tokenId) external override ReentrancyGuard {
        ...
        address paymentToken = address(uint160(tokenId));
        IERC20(paymentToken).transfer(user, pending);
    ...
    function setTxOriginFeeShare(uint16 initiatorFeeShareBp) external {
        //hardcap
        if( initiatorFeeShareBp > 100 ){
            revert InitatorFeeBoundExceeded();
        }
        txOriginFeeShareBasisPoints = initiatorFeeShareBp;
        emit InitiatorFeeChange(initiatorFeeShareBp);  
    }
    function setTokenSaleStakingContract(address _contractAddress, uint16 _feeShareBp) external onlyOwner {
        ...
        tokenSaleStakingContract = ITokenSaleStaking(_contractAddress); // Cast to the correct interface
        tokenSalePoolFeeShareBasisPoints = _feeShareBp;
        emit TokenSaleStakingContractConnected(_contractAddress, _feeShareBp);
    }
    // --- IERC1155Receiver ---
