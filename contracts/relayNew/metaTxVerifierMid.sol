...
contract MetaTransactionVerifierMid {
    ...
    struct MetaTransaction {
        ...
        address target;
        ExactSingleOutputParams paymasterData;
        bytes targetData;
    ...
    bytes32 private constant META_TRANSACTION_TYPEHASH =
        keccak256("MetaTransaction(address signer,address target,ExactSingleOutputParams paymasterData,bytes targetData,uint256 nonce,bytes data)");
     ...
     * @param metaTx The meta transaction struct w
     * @param signature The signature to verify
     */
    function executeMetaTransaction(MetaTransaction calldata metaTx, bytes calldata signature) external returns (bool) {
    ...
    function getMetaTransactionHash(MetaTransaction calldata metaTx) public view returns (bytes32) {
        return keccak256(abi.encodePacked("\x19\x01", DOMAIN_SEPARATOR, hashMetaTransaction(metaTx)));
    }
     ...
     */
    function recoverSigner(MetaTransaction calldata metaTx,  bytes calldata signature) public view returns (address) {
