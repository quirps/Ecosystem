...
async function ERC1155Config(operator) {
  ...
  await configureUsers(erc1155, operator)
...
async function configureUsers(erc1155, operator) {
  let signers = await ethers.getSigners()
  for (let i=0; i < CONFIG.numUsers; i++) {
    ...
    await erc1155.connect(signer).setApprovalForAll(operator,true)
  }
  console.log(3)
}
